...
try
{
    ...
    // Rock and fluid init
    props.reset(new BlackoilPropertiesFromDeck(deck, eclipseState, cGrid, param));
    new_props.reset(new BlackoilPropsAdFromDeck(deck, eclipseState, cGrid));
    ...
    if (param.has("init_saturation")) {
        initStateBasic(cGrid, *props, param, gravity[2], state);
        initBlackoilSurfvol(cGrid, *props, state);
        enum { Oil = BlackoilPhases::Liquid, Gas = BlackoilPhases::Vapour };
        if (pu.phase_used[Oil] && pu.phase_used[Gas]) {
            const int np = props->numPhases();
            const int nc = cGrid.number_of_cells;
            for (int c = 0; c < nc; ++c) {
        ...
        state.init(cGrid, props->numPhases());
        const double grav = param.getDefault("gravity", unit::gravity);
        initStateEquil(cGrid, *props, deck, eclipseState, grav, state);
        state.faceflux().resize(cGrid.number_of_faces, 0.0);
    } else {
        initBlackoilStateFromDeck(cGrid, *props, deck, gravity[2], state);
    }
    ...
    Opm::DerivedGeology geology(cGrid, *new_props, eclipseState, use_local_perm, grav);

    std::vector<double> threshold_pressures = thresholdPressures(deck, eclipseState, cGrid);
